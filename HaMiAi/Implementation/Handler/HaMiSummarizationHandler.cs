...
public class HaMiSummarizationHandler
    : IPipelineStepHandler
{
    ...
    public HaMiSummarizationHandler(
        string stepName,
    ...
    private async Task<(string summary, bool skip)> SummarizeAsync(string content, IContext context)
    {
        ...
        while (!done)
        {
            ...
            for (int index = 0; index < paragraphs.Count; index++)
            {
                ...
                try
                {
                    await foreach (string token in textGenerator.GenerateTextAsync(filledPrompt, new Microsoft.KernelMemory.AI.TextGenerationOptions()).ConfigureAwait(false))
                    {
                ...
                {
                    this._log.LogWarning("Summarization canceled {message}", ex.Message);
                    return (content, true);
                }
